...
/**
 * SF关联模型扩展
 * Update time：2015-11-18 23:37:21
 */
        ...
        if (!empty($this->_link)) {
            foreach ($this->_link as $key => $val) {
                ...
                if (empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName, $name))) {
                    ...
                    $mappingCondition = !empty($val['condition']) ? $val['condition'] : ''; // 关联条件
                    ...
                    switch ($mappingType) {
                        ...
                        case self::MANY_TO_MANY:
                            ...
                            $mappingOrder = !empty($val['mapping_order']) ? $val['mapping_order'] : '';
                            $mappingLimit = !empty($val['mapping_limit']) ? $val['mapping_limit'] : '';
                            $mappingRelationFk = !empty($val['relation_foreign_key']) ? $val['relation_foreign_key'] : $model->getModelName() . '_id';
                        ...
                        if (isset($val['as_fields']) && in_array($mappingType, array(self::HAS_ONE, self::BELONGS_TO))) {
                            ...
                            foreach ($fields as $field) {
                                if (strpos($field, ':')) {
                                    ...
                                    $result[$nick]             = isset($relationData[$relationName]) ? $relationData[$relationName] : null;
                                } else {
                                    $result[$field] = isset($relationData[$field]) ? $relationData[$field] : null;
                                }
                            }
                        } else {
                    ...
                    if (!empty($mappingData) || 'DEL' == $opType) {
                        switch ($mappingType) {
                            ...
                            case self::HAS_MANY:
                                ...
                                break;
                            case self::MANY_TO_MANY:
                                $mappingRelationFk = !empty($val['relation_foreign_key']) ? $val['relation_foreign_key'] : $model->getModelName() . '_id'; // 关联
                                $prefix            = $this->tablePrefix;
                                if (isset($val['relation_table'])) {
                                    $mappingRelationTable = preg_replace_callback("/__([A-Z_-]+)__/sU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $val['relation_table']);
                                } else {
