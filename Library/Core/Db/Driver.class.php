...
/**
 * SF 数据库驱动基类
 * Update time：2015-10-30 16:42:46
 */
abstract class Driver {

    protected $PDOStatement         = null;             // PDO操作实例
    protected $model                = '_SF_';           // 当前操作所属的模型名
    protected $queryStr             = '';               // 当前模型最近一条SQL指令
    protected $modelSql             = array();          // 所有模型最近的一条SQL指令
    protected $lastInsID            = null;             // 最后插入ID
    protected $numRows              = 0;                // 最近一次操作所影响的记录数
    ...
    protected $queryTimes   = array();                  // 查询计数
    protected $options      = array(                    // PDO连接参数
    ...
    public function execute($str, $fetchSql = false) {
        ...
        if (preg_match("/^\s*(SELECT|SHOW)\s+/i", $str)) {
            $sql_type = 'select';
        } elseif (preg_match("/^\s*DELETE\s+/i", $str)) {
            $sql_type = 'delete';
        } elseif (preg_match("/^\s*UPDATE\s+/i", $str)) {
            $sql_type = 'update';
        } elseif (preg_match("/^\s*(INSERT\s+INTO|REPLACE\s+INTO)\s+/i", $str)) {
            $sql_type = 'insert';
        } else {
            E('SQL 语法低级错误，类型无法识别：' . $str); // 更多操作类型暂不支持（创建表、修改表结构……）
        }

        N('db_' . $sql_type, 1);

        if (isset($this->queryTimes[$this->model][$sql_type]))
            $this->queryTimes[$this->model][$sql_type]++;
        else
            $this->queryTimes[$this->model][$sql_type] = 1;
    ...
    public function insert($data, $options = array(), $replace = false) {
        ...
        empty($options['bind']) or $this->parseBind($options['bind']);
        $values = $fields = array();
        foreach ($data as $key => $val) {
        ...
        empty($options['comment']) or $sql .= $this->parseComment($options['comment']);
        return $this->execute($sql, !empty($options['fetchsql']) ? true : false);
    }
    ...
    public function select($options = array()) {
        ...
        empty($options['bind']) or $this->parseBind($options['bind']);
        $sql    = $this->buildSelectSql($options);
        $result = $this->execute($sql, !empty($options['fetchsql']) ? true : false);
        return $result;
    }
    ...
    public function parseSql($sql, $options = array()) {
        $sql = str_replace(
            ...
            array(
                ...
                !empty($options['distinct']) ? $this->parseDistinct($options['distinct'])   : '',
                ...
                !empty($options['union'])    ? $this->parseUnion($options['union'])         : '',
                !empty($options['lock'])     ? $this->parseLock($options['lock'])           : '',
                !empty($options['comment'])  ? $this->parseComment($options['comment'])     : '',
                !empty($options['force'])    ? $this->parseForce($options['force'])         : '',
            ), $sql);
        return $sql;
    }
     ...
     * @return integer
     */
    public function getQueryTimes($model = '', $type = '') {
        if (empty($model)) $model = $this->model;
        return $type == '' ? array_sum($this->queryTimes[$model]) : $this->queryTimes[$model][$type];
    }
    ...
    public function error($e = null) {
        if ($this->PDOStatement) {
            ...
            if (is_object($e)) {
                $error_[1] = $e->getMessage() ?: '未知';
                $error_[2] = $e->getCode() ?: '未知';
            } else {
                $error_[1] = '未知';
                $error_[2] = '未知';
            }

            $this->error =  L('_SQL_ERROR_') . ' : [' . ($error[1]?:$error_[2]) . '] ' . ($error[2]?:$error_[1]);
        } else {
            $this->error = L('_SQL_ERROR_');
        }
        if ('' != $this->queryStr) {
            $this->error .= "\r\nSQL QUERY : " . $this->queryStr;
        }
        trace($this->error, 'ERROR', true);
    ...
    public function setModel($model) {
        $this->model = !empty($model) ? $model : '_SF_';
    }
     ...
     * @param string
     */
    protected function debug($start, $sql_type = '') {
        if ($this->config['debug']) {
            if ($start) {
                G('queryStartTime');
            } else {
                ...
                G('queryEndTime');
                $executeTime = G('queryStartTime', 'queryEndTime');
                N('db_' . $sql_type . '_executeTime', $executeTime);
                trace('[' . $this->model . '] ' . $this->queryStr . ' (executeTime: ' . $executeTime . 's )', 'SQL');
            }
        }
    }
