...
class Model {
    ...
    protected $methods = array('strict', 'order', 'alias', 'having', 'group', 'lock', 'distinct', 'auto', 'filter', 'validate', 'result', 'token', 'index', 'force', 'comment', 'fetchsql');   // 链操作方法列表
    ...
    public function __call($method, $args) {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            return $this->getField(strtoupper($method) . '(' . $field . ') AS sf_' . $method);
    ...
    protected function _checkTableInfo() {
        if (empty($this->fields)) {
            if (C('DB_FIELDS_CACHE')) {
                $fields = F('Cache/fields/' . strtolower(($this->dbName ?: C('DB_NAME')) . '.' . $this->tablePrefix . $this->name));
                if ($fields) {
                    $this->fields = $fields;
                    if (!empty($fields['_pk'])) {
                        $this->pk = $fields['_pk'];
    ...
    public function flush() {
        $this->fields = $this->getDbFieldsInfo();
        if (C('DB_FIELDS_CACHE'))
            F('Cache/fields/' . strtolower(($this->dbName ?: C('DB_NAME')) . '.' . $this->tablePrefix . $this->name), $this->fields);
    }
    ...
    protected function _parseOptions($options = array()) {
        ...
        if (!empty($options['alias']) && is_string($options['table'])) {
        ...
        if (isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            foreach ($options['where'] as $key => $val) {
                ...
                if (in_array($key, $fields, true)) {
                    if (is_scalar($val) && isset($fields['_type'])) {
    ...
    protected function _facade($data) {
        ...
        if (!empty($this->fields)) {
            ...
            foreach ($data as $key => $val) {
                if (!in_array($key, $fields, true)) {
                    ...
                    if(!$fields['_notnull'][$key])
                        E(L('_DATA_TYPE_INVALID_') . ' : ' . $key . '=>NULL');
                }
            }
        }
        ...
        if (!empty($this->options['filter'])) {
            ...
            $data = arrayMapRecursive($this->options['filter'], $data);
    ...
    public function add($data = '', $options = array(), $replace = false) {
        ...
        if (false !== $result && is_numeric($result)) {
            ...
            if ($insertId) {
                $data[$pk] = $insertId;
                if (false === $this->_after_insert($data, $options)) return false;
                return $insertId;
            }
            if (false === $this->_after_insert($data, $options)) return false;
        }
    ...
    public function delete($options = array()) {
        ...
        if (empty($options) && empty($this->options['where'])) {
            if (!empty($this->data) && isset($this->data[$pk])) {
                ...
                $this->error = L('_NO_DELETE_EXPRESS_');
                return false;
            }
        }
        if (is_numeric($options) || is_string($options)) {
            ...
            if (count($pk) == $count) {
                ...
                $this->error = L('_ERROR_UPDATA_KEY_');
                return false;
            }
        }
            ...
            if (is_string($pk)) {
                ...
                foreach ($pk as $field) {
                    if (isset($options['where'][$field]))
                        $data[$field] = $options['where'][$field];
                }
            }
        }
        ...
        if (false !== $result && is_numeric($result)) {
            $this->_after_delete(isset($data) ? $data : array(), $options);
        }
        return $result;
    }
    ...
    public function select($options = array())  {
        ...
        $data = array_map(array($this, '_read_data'), $resultSet);

        $this->_after_select($data, $options);
    ...
    public function table($table) {
        ...
        if (is_array($table)) {
            ...
            $this->options['table'] = preg_replace_callback("/__([A-Z0-9_-]+)__/sU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $table);
        }
        return $this;
    }
     ...
     * 返回模型的错误信息
     * @access public
     * @param boolean $k  确保能返回错误信息
     * @return string
     */
    public function getError($k = false) {
        return $this->error ? : ($k ? $this->db->getError() : '');
    }
     ...
     * @return string
     */
    public function getQueryTimes($type = '') {
        return $this->db->getQueryTimes($this->name, $type);
    }
    ...
    public function getDbFields() {
        if (isset($this->options['table'])) {
            if (is_array($this->options['table'])) {
                ...
                if (is_numeric($table))
                    $table = $this->options['table'][$table];
            } else {
                $table = $this->options['table'];
                list($table) = explode(',', $table);
                list($table) = explode(' ', trim($table));
                if (strpos($table, ')')) {
                    return false;
                }
            }
        ...
        if ($this->fields) {
            $fields = $this->fields;
            unset($fields['_pk'], $fields['_type'], $fields['_notnull']);
            return $fields;
        }
        return false;
    }
    ...
    public function getDbFieldsInfo() {
        if (!$this->fields) {
            ...
            foreach ($fields as $key => $val) {
                $this->fields['_type'][$key]    = $val['type'];
                $this->fields['_notnull'][$key] = $val['notnull'];
                if ($val['primary']) {
